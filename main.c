...
int SMTick1(int state){
	...
	switch(state){
		case SM1_Write:
			if(cnt > 0){cnt--;}
			else{output1 = ~output1; cnt = setCnt;}
		break;
...
int SMTick2(int state2){
	...
	switch(state2){
		case SM2_Write:
			if(cnt > 0){cnt--;}
			else{output2 = ~output2;cnt = setCnt;}
		break;
	...
	switch(state4){
		case SM4_Write:
			if(cnt > 0){cnt--;}
				...
				if(output4 == 0x80){output4 = 0x01;}
				else{output4 = output4 << 1 ;}
				cnt = setCnt;
			}
...
int main(void)
{
	...
	//Tasks
		...
		unsigned long int SMTick5_period = 50;
	...
	unsigned char prevReceivedData = receivedData;
	setCnt = 1;
	cnt = 1;
	SPI_ServantInit();
    ...
    while (1) {
			if((receivedData & 0x0F) == 1){
				if(receivedData != prevReceivedData){setCnt = 39; cnt = setCnt;}
			}
			else if((receivedData & 0x0F) == 2){
				if(receivedData != prevReceivedData){setCnt = 19; cnt = setCnt;}
			}
			else if((receivedData & 0x0F) == 3){
				if(receivedData != prevReceivedData){setCnt = 9; cnt = setCnt;}
			}
			else if((receivedData & 0x0F) == 4){
				if(receivedData != prevReceivedData){setCnt = 4; cnt = setCnt;}
			}
			else if((receivedData & 0x0F) == 5){
				if(receivedData != prevReceivedData){setCnt = 1; cnt = setCnt;}
			}
			else if((receivedData & 0x0F) == 6){
				if(receivedData != prevReceivedData){setCnt = 0; cnt = setCnt;}
			}
